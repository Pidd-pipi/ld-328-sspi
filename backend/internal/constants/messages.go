...
const (
	...
	MsgDisposalNotAllowed      = "仅临期（expiring）或已过期（expired）食品可发起处置申请"
	MsgDisposalDuplicate       = "该食品（FoodItem）已存在待处理（pending）处置申请"
	MsgDisposalQuantity        = "DisposalApplication 申请数量超出余量，整单拒绝"
	MsgDisposalConsumed        = "食品（FoodItem）已消耗（consumed），处置申请整单拒绝"
	MsgDisposalNotPending      = "处置申请（DisposalApplication）已结案，不能重复审批"
	MsgDisposalNotFound        = "处置申请（DisposalApplication）不存在"
	MsgDisposalReviewForbidden = "仅家庭管理员（FamilyMember.role=admin）可批准或驳回处置申请"
